...
$(document).ready(function () {
    ...
    */
    ...
    $('#Subverse').autocomplete(
        {
            source: '/index.php?view=rest&entry=autocompletenodename'
        });
    ...
    window._bd_share_config={
        ...
        "share":[
            {"tag":"list","bdSize":"16","bdCustomStyle":"/templates/css/share_style2_16.css"},
            {"tag":"view","bdSize":"32","bdCustomStyle":"/templates/css/share_style2_32.css"}
        ]
    };
    with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src='http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion='+~(-new Date()/36e5)];
    $('.bdsharebuttonbox').mouseover(setShare)
});
...
function submitUpVote(messageid) {
    $.ajax({
        type: "POST",
        url: "/index.php?view=feed&entry=vote_feed&feed_id=" + messageid + "&score=1"
    });
}
...
function voteUpComment(commentid) {
    ...
    // get current score
    var scoreLikes = +($(".commentid-" + commentid).find('.post_upvotes').filter(":first").html());
    var scoreDislikes = -($(".commentid-" + commentid).find('.post_downvotes').filter(":first").html());

	// ADD LIKE IF UNVOTED
    if ($(".commentid-" + commentid).children(".midcol").is(".unvoted")) {
        $(".commentid-" + commentid).children(".midcol").toggleClass("likes", true); //add class likes
        $(".commentid-" + commentid).children(".midcol").toggleClass("unvoted", false); //remove class unvoted
        // add upvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvote").toggleClass("arrow-upvoted", true); //set upvote arrow to upvoted
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvote").toggleClass("arrow-upvote", false); //remove upvote arrow
        // increment comment points counter and update DOM element
        scoreLikes++;
        $(".commentid-" + commentid).find('.post_upvotes').filter(":first").html('+' + scoreLikes);
        $(".commentid-" + commentid).find('.score.unvoted').filter(":first").html((scoreLikes - scoreDislikes) + " points");
        $(".commentid-" + commentid).find('.score.onlycollapsed').filter(":first").html((scoreLikes - scoreDislikes) + " points");
    } else if ($(".commentid-" + commentid).children(".midcol").is(".likes")) {
        // REMOVE LIKE IF LIKED
        $(".commentid-" + commentid).children(".midcol").toggleClass("unvoted", true); //add class unvoted
        $(".commentid-" + commentid).children(".midcol").toggleClass("likes", false); //remove class dislikes
        // remove upvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvoted").toggleClass("arrow-upvote", true); //set arrow to upvote
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvoted").toggleClass("arrow-upvoted", false); //remove upvoted arrow
        // decrement comment points counter and update DOM element
        scoreLikes--;
        $(".commentid-" + commentid).find('.post_upvotes').filter(":first").html('+' + scoreLikes);
        $(".commentid-" + commentid).find('.score.unvoted').filter(":first").html((scoreLikes - scoreDislikes) + " points");
        $(".commentid-" + commentid).find('.score.onlycollapsed').filter(":first").html((scoreLikes - scoreDislikes) + " points");
    } else if ($(".commentid-" + commentid).children(".midcol").is(".dislikes")) {
        // ADD LIKE IF DISLIKED
        $(".commentid-" + commentid).children(".midcol").toggleClass("dislikes", false); //remove class dislikes
        $(".commentid-" + commentid).children(".midcol").toggleClass("likes", true); //add class likes
        $(".commentid-" + commentid).children(".midcol").toggleClass("unvoted", false); //remove class unvoted
        // remove downvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvoted").toggleClass("arrow-downvote", true); //set downvoted arrow to downvote
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvoted").toggleClass("arrow-downvoted", false); //remove downvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvote").toggleClass("arrow-upvoted", true); //add upvoted arrow
        // increment/decrement comment points counters and update DOM element
        scoreLikes++;
        scoreDislikes--;
        $(".commentid-" + commentid).find('.post_upvotes').filter(":first").html('+' + scoreLikes);
        $(".commentid-" + commentid).find('.post_downvotes').filter(":first").html('-' + scoreDislikes);
        $(".commentid-" + commentid).find('.score.unvoted').filter(":first").html((scoreLikes - scoreDislikes) + " points");
        $(".commentid-" + commentid).find('.score.onlycollapsed').filter(":first").html((scoreLikes - scoreDislikes) + " points");
    }
}
...
function voteDownComment(commentid) {
	...
	// ADD DISLIKE IF UNVOTED
    if ($(".commentid-" + commentid).children(".midcol").is(".unvoted")) {
        $(".commentid-" + commentid).children(".midcol").toggleClass("dislikes", true); //add class dislikes
        $(".commentid-" + commentid).children(".midcol").toggleClass("unvoted", false); //remove class unvoted
        // add downvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvote").toggleClass("arrow-downvoted", true); //set downvote arrow to downvoted
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvote").toggleClass("arrow-downvote", false); //remove downvote arrow
        // increment comment points counter and update DOM element        
        scoreDislikes++;
        $(".commentid-" + commentid).find('.post_downvotes').filter(":first").html('-' + scoreDislikes);
        $(".commentid-" + commentid).find('.score.unvoted').filter(":first").html((scoreLikes - scoreDislikes) + " points");
        ...
        $(".commentid-" + commentid).children(".midcol").toggleClass("dislikes", false); //remove class dislikes
        // remove downvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvoted").toggleClass("arrow-downvote", true); //set arrow to downvote
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvoted").toggleClass("arrow-downvoted", false); //remove downvoted arrow
        // decrement comment points counter and update DOM element
        scoreDislikes--;
        $(".commentid-" + commentid).find('.post_downvotes').filter(":first").html('-' + scoreDislikes);
        ...
        // ADD DISLIKE IF LIKED
        $(".commentid-" + commentid).children(".midcol").toggleClass("likes", false); //remove class likes
        $(".commentid-" + commentid).children(".midcol").toggleClass("dislikes", true); //add class dislikes
        $(".commentid-" + commentid).children(".midcol").toggleClass("unvoted", false); //remove class unvoted
        // remove upvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvoted").toggleClass("arrow-upvote", true); //set upvoted arrow to upvote
        $(".commentid-" + commentid).children(".midcol").children(".arrow-upvoted").toggleClass("arrow-upvoted", false); //remove upvoted arrow
        $(".commentid-" + commentid).children(".midcol").children(".arrow-downvote").toggleClass("arrow-downvoted", true); //add downvoted arrow
        // increment/decrement comment points counters and update DOM element
        scoreLikes--;
        scoreDislikes++;
        $(".commentid-" + commentid).find('.post_upvotes').filter(":first").html('+' + scoreLikes);
...
function submitCommentUpVote(commentid) {
    $.ajax({
        type: "POST",
        url: "/index.php?view=feed&entry=vote_comment&comment_id=" + commentid + "&score=1"
    });
}
...
function reply(parentcommentid, messageid) {
    ...
    $('#replyform-'+parentcommentid).slideToggle();
}
function reply_old(parentcommentid, messageid) {
    //exit function if the form is already being shown
...
function removereplyform_old(parentcommentid) {
...
function reportcomment(obj, commentid) {
    ...
    $.ajax({
        ...
        error: function () {
            $(obj).parent().parent().find('.togglebutton').html("好像哪里出问题了");
        }
    });
...
function subscribe(obj, subverseName) {
    ...
    $.ajax({
        type: "POST",
        url: "/index.php?view=node&entry=subscribe&node_id=" + subverseName,
        success: function () {
...
function suggestTitle() {
    ...
    var title = $.get(
        "/index.php?view=rest&entry=titlefromuri&uri=" + uri,
        null,
        function (data) {
            $("#Linkdescription").val(data);
            $("#suggest-title").text("在上面输入网址，点击这里获取标题");
            $("#suggest-title").on('click', suggestTitle);
        }).fail(function () {
            $("#Linkdescription").val("获取标题失败");
            $("#suggest-title").text("在上面输入网址，点击这里获取标题");
            $("#suggest-title").on('click', suggestTitle);
        });
}
